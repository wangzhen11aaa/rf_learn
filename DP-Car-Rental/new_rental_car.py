...
class RentalBranch:
    ...
    def transfer_cars(self, car_transfers):
        ...
        if car_transfers < 0:
            ...
            self.overflow = car_transfers > free_spots
            ...
            for qA in range(totalA+1):
                ...
                for totalB in range(max_capacity+1):
                    for qB in range(totalB+1):
                        ...
                        self.value_map[State(branchA, branchB)] = 0
...
def estimate_action_value(state, action, value_map):
   ...
   # branchA = state.branchA
    branchB = copy.copy(state.branchB)

    # Transfer cars and record transfer costs, if we transfer cars from A to B, then the cost will be less for the free transfer
    if action > 0:
        cost_action = action - 1
    else:
        cost_action = -action
    #income = - TRANSFER_COST * abs(cost_action) # We always pay so it's always non-positive
    income = - TRANSFER_COST * action # We always pay so it's always non-positive
    try:
        branchA.transfer_cars(-action)
        branchB.transfer_cars(action)
        if branchA.overflow or branchB.overflow:
            income -= 4
...
def print_policy(policy):
    ...
    for availableA in range(max_capacity, -1, -1):
        ...
        transfers = [str(policy[s]) for s in states]
