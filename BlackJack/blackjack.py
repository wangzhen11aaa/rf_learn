...
def best_policy(value_function, player, dealer):
    # We evaluate value_function, defaultdict type, well done
...
def iteration(iterations, update, Name, policy, n_zero=100):
    ...
    for j in xrange(iterations):
        ...
        while not game.terminal:
            ...
            # and take new action for player1
            if action1 is not 1 and reward1 is not -1:
                # One state occures more time, It is more important because of access times from player1 and player2.
                # so we prefer to choosing much proper action (policy learned before)
                epsilon = n_zero / float(n_zero + counter_state[(player1, dealer)])
                action1 = policy(epsilon, value_function, player1, dealer)

            else:
                action1 = 1
                # Why player2 does not take epsilon-greedy action like player1
                action2 = random_policy()

            # Only the action upon the state of player1 will be counted and only one time, and player may not win
            if (player1, dealer, action1) not in observed_keys1 and player1 <=21:
                observed_keys1.append((player1, dealer, action1))
            # Compared with player1, the action upon state of player2 is more free.
        ...
        # We count the wins from the tail episodes,when the policy is almost optimal
        if j > iterations * 0.8:
            if reward1 == 1:
                wins1 += 1
            if reward2 == 1:
    ...
    if reward1 is not None:
        ...
        for i in range(len(observed_keys1)):
            ...
            if i < len(observed_keys1) - 1:
                ...
                new = 0
            # Update this situation with the optimal value of next situation current
    ...
    if reward1 is not None:
        ...
        for i in range(len(observed_keys1)):
            ...
            if i < len(observed_keys1) - 1:
                ...
                new = 0
            # TD Learning,update the state_value with the following state_value in this episode
            value_function[observed_keys1[i]] = (1-alpha) * old + alpha * (reward1 + new)
